...
class SettingsView(LoginRequiredMixin, FormView):
    ...
    def get_initial(self):
        ...
        initial["street_address"] = self.request.user.account.street_address
        initial["city"] = self.request.user.account.city
        initial["state"] = self.request.user.account.state
        initial["zip_code"] = self.request.user.account.zip_code
        initial["main_phone"] = self.request.user.account.main_phone
        initial["alt_phone"] = self.request.user.account.alt_phone
        return initial
    ...
    def update_account(self, form):
        ...
        fields["zip_code"] = form.cleaned_data["zip_code"]
        fields["main_phone"] = form.cleaned_data["main_phone"]
        fields["alt_phone"] = form.cleaned_data["alt_phone"]
        if fields:
    ...
    def update_user(self, form):
        ...
        if fields:
            user_fields = self.request.user
            for k, v in fields.items():
                setattr(user_fields, k, v)
            user_fields.save()
